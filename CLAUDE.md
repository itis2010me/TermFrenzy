...
- **NPCFish** — classmethod `spawn()` factory (weighted: small fish spawn 3x more via `NPC_SPAWN_WEIGHTS`). Each fish gets a `color_name` on spawn: small fish (level 0) are randomly orange or blue (`SMALL_COLORS`), medium fish (level 1) are purple (`bright_magenta`). Methods: `update()` (movement/bob/flee from threats, returns False if off-screen), `check_eat_collision()` (returns points or None), `draw()`. All NPC fish flee from predators: player (if eatable), larger NPC fish (`NPC_CAN_EAT`), and active sharks within `NPC_FLEE_RADIUS`.
