...
- **Player** — state: `px`, `py`, `facing_right`, `size`, `score`, `draw_x`, `draw_y`, `fish_w`, `fish_h`, `sprite`, `dropping`, `stung_until`, `gold_frenzy_until`. Has a drop-in animation on spawn (`DROP_DURATION = 1.0s`, ease-out curve) — mouse tracking starts after the drop completes. When stung by jellyfish, movement slows to 30% for 1s with a 0.3s wavy `~` flash effect. Gold frenzy state tracks the power-up timer. Methods: `update()`, `check_growth()`, `apply_sting()`, `is_stung()`, `activate_gold_frenzy()`, `is_gold_frenzy()`, `draw()`.
